...
class D3QuadraticLine extends React.Component {
    constructor(props) {
        super(props)
        this.quadraticLine = {
            width: null,
            height: null,
            fy: (x, a = 1, b = 0, c = 0) => a * x * x + b * x + c // y=ax²+bx+c（且a≠0）
        }
    }
    ...
    componentDidMount() {
        ...
        d3.range(-10, 10, 0.05).forEach(function (item) {
            ...
            const itemY = formatXY(fy(itemX, a, b, c))
    ...
    render() {
        return (
            <div className="quadratic-line-simple">
...
D3QuadraticLine.propTypes = {
    a: PropTypes.number,
    b: PropTypes.number,
    c: PropTypes.number
}

export default D3QuadraticLine
